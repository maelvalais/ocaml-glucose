...
(*
 * Processes the content of [file], adds the variables and clauses to Glucose
 * and returns a mapping between names and Glucose variables.
 *)
let process_file (file: in_channel): (string, Glucose.var) Hashtbl.t =

  (* Mapping between variable names and indices. *)
  let vars : (string, Glucose.var) Hashtbl.t =
    Hashtbl.create 0
  in
...
let solve (file: in_channel): unit =
  ...
  match Glucose.solve () with
  | Glucose.UNSAT -> printf "unsat\n"
  | Glucose.SAT   ->
      printf "sat\n";
      Hashtbl.iter
        (fun name v ->
          printf "  %s=%s\n"
            name
            (Glucose.string_of_value (Glucose.value_of v))
        )
        vars
